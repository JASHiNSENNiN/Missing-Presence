...
func test_saveload_in_large_converted_act() -> void:
	Dialogic.start("res://Dialogue/Acts/Act5/act5_good.dtl")
	await wait_frames(4)
...
const HALLWAY_BG := "res://Art/Backgrounds/campus_hallway.png"


func test_saveload_restores_background() -> void:
	Dialogic.start("res://Dialogue/Acts/Act1/act1_hallway.dtl")
	var bg_before := ""
	for _i in 30:
		await wait_frames(1)
		...
		if bg_before == HALLWAY_BG:
			break
	assert_eq(bg_before, HALLWAY_BG, "hallway [background] event should set its own background")
	...
	assert_eq(bg_after, HALLWAY_BG, "background should restore to the saved hallway image")
...
func test_multiple_saveload_cycles_stable() -> void:
	Dialogic.start("res://Dialogue/Acts/Act2/act2_dinner.dtl")
	await wait_frames(4)
	for i in 3:
...
func test_route_progress_persists_across_acts() -> void:
	Dialogic.start("res://Dialogue/Acts/Act4/act4_good.dtl")
	await wait_frames(4)
	Dialogic.VAR.Route.self_honesty = 3
	Dialogic.VAR.Route.affinity_ethan = 1
	...
	await GameFlow.load_slot_and_wait(SLOT)
	assert_eq(int(Dialogic.VAR.Route.self_honesty), 3, "route progress must survive save/load")
	assert_eq(int(Dialogic.VAR.Route.affinity_ethan), 1, "affinity must survive save/load")
